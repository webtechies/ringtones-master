...
- (void)tableView:(UITableView *)tableView hideVisibleBackView:(BOOL)animated {
	...
	if ([cell respondsToSelector:@selector(hideBackViewAnimated:)]){
		[(TISwipeableTableViewCell *)cell hideBackViewAnimated:YES];
...
@synthesize swipeCellStatus = _swipeCellStatus;
...
- (void)initialSetup {
    ...
    UISwipeGestureRecognizer * swipeRightRecognizer = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(cellWasSwiped:)];
...
- (void)cellWasSwipedLeft:(UISwipeGestureRecognizer *)recognizer
{
    ...
    {
        _swipeCellStatus = 0;
        [self revealBackViewAnimatedRight:YES];
    }
    
    if (_swipeCellStatus == 2)
    {
        _swipeCellStatus = 1;
        [self hideBackViewAnimated:YES];
    }
...
- (void)revealBackViewAnimated:(BOOL)animated {
	...
	if (!_contentViewMoving && _backView.hidden){
		...
		[_contentView.layer setPosition:CGPointMake( _contentView.frame.size.width / 2, _contentView.layer.position.y)];
...
- (void)revealBackViewAnimatedRight:(BOOL)animated {
	...
	if (!_contentViewMoving && _backView.hidden){
		...
		[_contentView.layer setAnchorPoint:CGPointMake(0, 0.5)];
		[_contentView.layer setPosition:CGPointMake( - 82, _contentView.layer.position.y)];
