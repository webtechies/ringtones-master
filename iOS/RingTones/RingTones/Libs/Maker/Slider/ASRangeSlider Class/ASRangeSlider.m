...
-(void) setValue : (float) value forThumb : (SliderThumbView *) thumb
{
    ...
    float duration = kRingtoneCutMinDuration;
    float alongValue = kRingtoneCutMinDuration;
    
//    float maxThumbValue, minThumbValue;
    if (thumb == thumbOne) {
//        NSLog(@"change thumbOne");
        duration = fabsf([self valueForThumb:thumbTwo] - toSet);
        alongValue = [self valueForThumb:thumbTwo] + toSet - [self valueForThumb:thumbOne];
    } else {
//        NSLog(@"change thumbTwo");
        duration = fabsf([self valueForThumb:thumbOne] - toSet);
        alongValue = [self valueForThumb:thumbOne] + toSet - [self valueForThumb:thumbTwo];
    }
...
//    duration = fabs(toSet - alongValue);
    duration = roundf(duration);
//    duration = (int) duration;
    if (duration >= kRingtoneCutMaxDuration) {
        duration = kRingtoneCutMaxDuration;
    }
    if (duration <= kRingtoneCutMinDuration) {
        duration = kRingtoneCutMinDuration;
    }
    
    float maxThumbValue = MIN(toSet, MAX([self valueForThumb:thumbOne],[self valueForThumb:thumbTwo]));
    
    float minThumbValue = MAX(toSet, MIN([self valueForThumb:thumbOne],[self valueForThumb:thumbTwo]));
    
    if (maxThumbValue >= durationPerScroll) {
        maxThumbValue = durationPerScroll;
    }
    if (minThumbValue <= 0.0f) {
        minThumbValue = 0.0f;
    }
//    NSLog(@"--->value1: %f -->>> value2: %f", [self valueForThumb:thumbOne], [self valueForThumb:thumbTwo]);
//	NSLog(@"toSet: %f, duration: %f, maxThumbValue: %f, minThumbValue: %f", toSet, duration, maxThumbValue, minThumbValue);

    
    
    //    && duration >= kRingtoneCutMinDuration && duration <= kRingtoneCutMaxDuration 
	if (toSet != [self valueForThumb:thumb] && maxThumbValue < durationPerScroll && minThumbValue > 0.0f)
	{
        CGFloat _tempDuration = fabsf(fabs(toSet) - fabs(alongValue));
        if (duration == kRingtoneCutMinDuration || duration == kRingtoneCutMaxDuration) {
            if (_tempDuration < kRingtoneCutMinDuration || fabs(toSet) <= 0.5 || toSet >= durationPerScroll || fabs(alongValue) <= 0.5 || alongValue >= durationPerScroll
...
//        NSLog(@"toset: %f, along: %f, duration: %f", toSet, alongValue, duration);
        ...
        if (duration == kRingtoneCutMinDuration || duration == kRingtoneCutMaxDuration) {
            ...
            [self setValue:alongValue forThumb:(thumb == thumbOne) ? thumbTwo : thumbOne];
        } 
//        else {
