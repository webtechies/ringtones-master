...
@synthesize labelLoadMore = _labelLoadMore;
...
- (UILabel *)titleLabel {
    if(!_labelLoadMore) {
        _labelLoadMore = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 210, 20)];
        _labelLoadMore.center = CGPointMake(self.bounds.size.width/2, self.bounds.size.height/2);
        _labelLoadMore.text = NSLocalizedString(@"Load more...",);
        _labelLoadMore.font = [UIFont helveticaNeueRegularFontSize:11];
        _labelLoadMore.backgroundColor = [UIColor clearColor];
        _labelLoadMore.textColor = [UIColor grayColor];
        [_labelLoadMore sizeToFit];
        _labelLoadMore.frame = CGRectMake(self.bounds.size.width/2 - _labelLoadMore.frame.size.width/2, self.activityIndicatorView.frame.origin.y + 20, _labelLoadMore.frame.size.width, _labelLoadMore.frame.size.height);
        [self addSubview:_labelLoadMore];
    }
    return _labelLoadMore;
}
