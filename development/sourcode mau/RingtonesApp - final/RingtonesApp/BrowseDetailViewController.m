...
@interface BrowseDetailViewController(privateMethods)
...
- (void)callLoadmore;
{
    ...
    NSString *param = @"";
    switch (sourceType) {
        case DataSourceTypeFeature:
            param = @"featured";
            break;
        case DataSourceTypeMostDownloaded:
            param = @"download";
            break;
        case DataSourceTypeTopRated:
            param = @"rate";
            break;
        case DataSourceTypeNewest:
            param = @"new";
            break;
        case DataSourceTypeMostCommented:
            param = @"comment";
            break;
        default:
            param = @"download";
    ...
    NSString *urlServer = [NSString stringWithFormat:SERVER_API_GETITEMBYTYPE, param, [arrSource count], kNoItemsPerPage];
...
//        [arrLoadmore addObject:[NSString stringWithFormat:@"cell More: %d", i]];
//    }
//    
//    
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    if (sourceCount == 0 && arrLoadmore == nil && isFirstLoad) {
        isFirstLoad = FALSE;
...
{
    NSLog(@"onUpgradeClicked browseDetail");
}

#pragma -Purchase delegate from App Delegate
- (void)purchaseDidFinish {
    NSLog(@"purchaseDidFinish browse detail");
}

- (void)purchaseDidFail {
    NSLog(@"purchaseDidFail browse detail");
}
